...
```bash
sudo setfacl -R -m u:devkraken:rwx /srv/http/renewal-ucr
sudo setfacl -R -m g:http:rwx /srv/http/renewal-ucr
```
## Set Default ACLs for New Files and Directories

To ensure that new files and directories created within the renewal-ucr directory automatically inherit the correct permissions:

```bash
sudo setfacl -d -m u:devkraken:rwx /srv/http/renewal-ucr
sudo setfacl -d -m g:http:rwx /srv/http/renewal-ucr
```
...
```bash
getfacl /srv/http/renewal-ucr
```

The output should look like this:

```text
# file: srv/http/renewal-ucr
# owner: http
# group: http
user::rwx
group::rwx
...
To verify that the settings are applied correctly, create a new file in the renewal-ucr directory:

```bash
touch /srv/http/renewal-ucr/newfile.txt
```
Then check the permissions of the new file:

```bash
getfacl /srv/http/renewal-ucr/newfile.txt
```
The output should show:

```text
# file: srv/http/renewal-ucr/newfile.txt
# owner: devkraken
# group: http
user::rwx
user:devkraken:rwx
...
By setting the default ACLs, you ensure that any new files or directories created within the renewal-ucr directory will automatically have the specified permissions. This approach helps maintain consistent access control for shared directories in a multi-user environment.